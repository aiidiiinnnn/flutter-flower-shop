...
class UserFlowerSearchCard extends GetView<UserFlowerSearchController> {
  UserFlowerSearchCard(
      {super.key, required this.searchFlower, required this.index});
  ...
  Obx searchCardAddButton(BuildContext context) {
    return Obx(() => controller.isAdded[searchFlower.id]!
        ? (controller.addToCartLoading[index])
            ? const SizedBox(
                width: 70, height: 10, child: LinearProgressIndicator())
            : Row(
  ...
  SizedBox searchCardDescription(BuildContext context) {
    return SizedBox(
      height: 35,
      width: 170,
      child: Container(
  ...
  void _showDescription(BuildContext context) {
    showDialog(
        ...
        builder: (BuildContext context) => AlertDialog(
              title: Text(
                  '${searchFlower.name} ${locale.LocaleKeys.user_description.tr}'),
        ...
        content: (searchFlower.count == 0)
            ...
            : Obx(() => Row(
                  ...
                  children: [
                    Row(
                      children: [
                        (controller.buyCounting[searchFlower.id] == 0)
                            ? _disableButton()
                            : decrementButton(),
                      ],
                    ),
  ...
  Widget addButton() {
    return InkWell(
        ...
        onTap: () => {controller.onTapAdd(flower: searchFlower, index: index)});
  }
