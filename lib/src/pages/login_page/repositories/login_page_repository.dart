...
class LoginPageRepository {
  ...
  Future<Either<String, List<LoginVendorViewModel>>> getVendors() async {
    ...
    if (response.statusCode >= 200 && response.statusCode < 400) {
      ...
      for (final vendor in vendorList) {
        final loginPageViewModel = LoginVendorViewModel.fromJson(vendor);
  ...
  Future<Either<String, List<LoginUserViewModel>>> getUsers() async {
    ...
    if (response.statusCode >= 200 && response.statusCode < 400) {
      ...
      for (final user in userList) {
        final loginPageViewModel = LoginUserViewModel.fromJson(user);
  ...
  Future<Either<String, List<LoginVendorViewModel>>> getVendorByEmailPassword({required String email, required String password}) async {
    final url = Uri.http(RepositoryUrls.fullBaseUrl, 'vendors', {'email':email,'password':password});
    final response = await http.get(url, headers: customHeaders);
  ...
  Future<Either<String, List<LoginUserViewModel>>> getUserByEmailPassword({required String email, required String password}) async {
    final url = Uri.http(RepositoryUrls.fullBaseUrl, 'users', {'email':email,'password':password});
    final response = await http.get(url, headers: customHeaders);
