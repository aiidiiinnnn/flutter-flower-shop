...
class SearchVendorFlowerController extends GetxController {
  ...
  final SearchVendorFlowerRepository _repository =
      SearchVendorFlowerRepository();
  int? vendorId;
  ...
  Rx<RangeValues> currentRangeValues = Rx<RangeValues>(const RangeValues(0, 1));
  Timer? deBouncer;
  ...
  Future<void> getFlowersByVendorId() async {
    if (disableLoading.value == false) {
      ...
      isLoading.value = true;
      isRetry.value = false;
      final Either<String, List<SearchVendorFlowerViewModel>> flower =
      ...
      flower.fold((left) {
        ...
        flowerSetValues(right);
      });
      isLoading.value = false;
    }
  }

  void flowerSetValues(List<SearchVendorFlowerViewModel> right) {
    ...
    if (searchList.isEmpty) {
      ...
      currentRangeValues =
          Rx<RangeValues>(RangeValues(minPrice.value, maxPrice.value));
  ...
  Future<void> addFlowerCount(
      {required SearchVendorFlowerViewModel flowerToEdit,
  ...
  SearchVendorFlowerDto _generatePlusDto(
          SearchVendorFlowerViewModel flowerViewModel) =>
      SearchVendorFlowerDto(
  ...
  SearchVendorFlowerDto _generateMinusDto(
          SearchVendorFlowerViewModel flowerViewModel) =>
      SearchVendorFlowerDto(
        name: flowerViewModel.name,
        description: flowerViewModel.description,
  ...
  Future<void> deleteFlower(
      SearchVendorFlowerViewModel flower, int index) async {
  ...
  Future<void> searchFlowers(String nameToSearch) async {
    ...
    isRetry.value = false;
    final Either<String, List<SearchVendorFlowerViewModel>> flower =
        await _repository.searchFlowers(query: query);
    flower.fold((left) {
      Get.snackbar(left, left);
      isLoading.value = false;
      isRetry.value = true;
  ...
  Future deleteFilter() async {
    ...
    isLoading.value = true;
    final Either<String, List<SearchVendorFlowerViewModel>> flower =
        await _repository.filteredFlower(query: query);
    flower.fold((left) {
      Get.snackbar(left, left);
      isLoading.value = false;
      isRetry.value = true;
